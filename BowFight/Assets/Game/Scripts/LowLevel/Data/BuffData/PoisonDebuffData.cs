...
public struct PoisonDebuffData : IBuffData
{
    public uint ID { get; private set; }
    public BuffType Type { get => BuffType.Poison; }
    public float PerTime { get => 0.5f; }
    public float StartTime { get; private set; }
    public float RemainTime { get; private set; }
    public string Thumbnail { get; private set; }
    public float AddValue { get; private set; }

    public PoisonDebuffData(SkillTableData skillData, float value, float currentTime)
    {
        if (skillData == null)
        {
